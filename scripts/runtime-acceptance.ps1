...
if (-not (Select-String -Path $WarriorLoadout -Pattern '^gold\t65535$' -Quiet)) {
    throw "Active RTT Warrior starting loadout does not contain gold=65535."
}
...
Write-Host "Create a fresh Warrior/Rogue/Sorcerer and confirm 65535 starting gold. Then reach Tristram, enter Cathedral Level 1, kill at least one monster, verify loot-filter/resistance UI behavior, save, then exit DevilutionX."
...
$Result.starterGoldObserved = Read-Pass "Did a fresh Warrior/Rogue/Sorcerer start with exactly 65535 gold?"
