...
- not covered by the headless test suite (needs real rendering to confirm placement visually); verified by a full clean re-application of all 23 patches from the pinned commit; the user's report that clicking the new objects showed no window/option at all could not be reproduced via static source review (dObject registration, selectionRegion, and OperateObject's dispatch all read correctly for every reused object type) - root cause found and fixed in patch 0024.

## Patch 0024 — Fix: town objects never responded to a mouse click

Purpose: fix the actual cause of patch 0023's open item - clicking the waypoint circle, stash chest, crafting anchor, or Horadric Cube did nothing, even though hovering them correctly showed their name.

Properties:

- root cause: `LeftMouseCmd()` in `diablo.cpp` has a dedicated early-return branch for `leveltype == DTYPE_TOWN` that only ever checked `pcursitem` (ground items) and `pcursmonst` (towners) before falling through to a walk command - it never checked `ObjectUnderCursor` at all. This isn't an RTT regression: vanilla town never had any operable `Object`s before RTT's physical waypoints/stash/crafting anchor/Horadric Cube existed, so nobody had reason to add the check there. The non-town branch immediately below it has always had the correct `ObjectUnderCursor != nullptr` -> `CMD_OPOBJXY` check - town's branch just never got the equivalent;
- confirmed the gamepad/controller equivalent (`Interact()` in `controls/plrctrls.cpp`) already falls through to its own `ObjectUnderCursor` check correctly for town (it only early-returns for `pcursmonst`), so no change was needed there - this bug was mouse-input-specific;
- fix: added the same `ObjectUnderCursor != nullptr` -> `CMD_OPOBJXY` check to the town branch, and excluded it from the walk-fallback condition so clicking an object no longer also queues a walk to that tile;
- additionally, `IsPlayerAdjacentToObject()` (`player.cpp`) now returns `true` unconditionally for any object `IsRttTownServiceObject()` identifies (stash chest/crafting anchor/Horadric Cube, not the waypoint circle) - these are UI-triggering props next to an NPC, not gameplay obstacles, so requiring the player to path all the way there and land on a precisely adjacent tile was an unnecessary second way for the same interaction to stall; the new `IsRttTownServiceObject()` (`objects.h`/`.cpp`) is a small public wrapper combining the 3 existing (internal-linkage) per-type checks, since the caller is in a different translation unit;
- presentation/input-only; no gameplay, save, or network behavior change beyond making the existing intended interactions actually reachable;
- not covered by the headless test suite (needs real mouse input); verified by a full clean re-application of all 24 patches from the pinned commit and by tracing the exact click -> `CMD_OPOBJXY` -> `OnObjectTileAction` -> `ACTION_OPERATE` -> `OperateObject()` call chain end-to-end in source, not just at the failure point - the user's next playtest is still needed to confirm the fix.
