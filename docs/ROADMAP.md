...
- [x] Rare engine coverage across the full base-item catalogue — `RttPhase4.RareRoundTripAcrossAllEquipmentTypes` round-trips Axe/Bow/Mace/Helm/MediumArmor/HeavyArmor/Ring/Amulet in addition to the original Sword/Staff/Shield/LightArmor prototypes; `RttPhase4.JewelryNeverGetsArmorClassAffix` proves Ring/Amulet never roll the armor-class affix family
- [x] Player-facing progression spend UI — F4 hotkey via `RttSpendNextProgressionPoint()` ([PR #29](https://github.com/wlff88/Return-to-Tristram/pull/29))
- [x] D2-style waypoint fast-travel panel — `RttWarpToLevel()` engine foundation ([PR #30](https://github.com/wlff88/Return-to-Tristram/pull/30)) plus a full graphical panel, M key in town ([PR #31](https://github.com/wlff88/Return-to-Tristram/pull/31))
